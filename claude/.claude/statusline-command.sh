...
# Set STATUSLINE_CHECK_UPDATES=false to disable the update check entirely.
...
cli_version_cache_file="/tmp/claude/statusline-cli-version-cache.txt"
cli_version_cache_max_age=3600  # 1 hour

cli_version=""
cli_version_needs_refresh=true

if [ -f "$cli_version_cache_file" ]; then
    cv_mtime=$(stat -c %Y "$cli_version_cache_file" 2>/dev/null || stat -f %m "$cli_version_cache_file" 2>/dev/null)
    cv_now=$(date +%s)
    cv_age=$(( cv_now - cv_mtime ))
    if [ "$cv_age" -lt "$cli_version_cache_max_age" ]; then
        cli_version_needs_refresh=false
    fi
    cli_version=$(cat "$cli_version_cache_file" 2>/dev/null)
fi

if $cli_version_needs_refresh; then
    touch "$cli_version_cache_file" 2>/dev/null
    cv_raw=$(claude --version 2>/dev/null | head -1)
    if [ -n "$cv_raw" ]; then
        # Extract just the version number (e.g. "1.0.71" from "Claude Code 1.0.71")
        cv_parsed=$(echo "$cv_raw" | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | head -1)
        if [ -n "$cv_parsed" ]; then
            cli_version="$cv_parsed"
            echo "$cli_version" > "$cli_version_cache_file"
        fi
    fi
fi

[ -n "$cli_version" ] && out+="${sep}${orange}claude@${cli_version}${reset}"
